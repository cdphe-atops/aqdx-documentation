...
The distance in meters from the inlet to the nearest obstruction (e.g., a wall, chimney, or sign) that is *not* the roof surface itself. If there are no nearby obstructions, use `null` or a large value (e.g., `999`).

### dist_roof_obstructions_m
**Format:** Decimal  
**Example:** `2.0`

If the instrument is on a roof, the distance in meters to the nearest obstruction *on* that roof.
